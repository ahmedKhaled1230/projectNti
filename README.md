...
| Field | Your answer |
|-------|-------------|
| **Full name** | Ahmed khaled Mohamed Youssri Khaled Elbohy |
...
If R3 runs before R1, the Garage lamp is turned on by R3 because its
temperature is above the alarm threshold. R1 then runs afterward and sees that
the Garage is unoccupied, so it clears the lamp again. The Garage still has
the alarm bit set, but its lamp ends up off because the later rule overwrites
